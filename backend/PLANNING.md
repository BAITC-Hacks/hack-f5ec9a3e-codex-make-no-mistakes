...
Реализован путь: подготовка исходных данных → расчёт → сохранение версии → корректировка количества с причиной → явное утверждение → CSV. Основной интерфейс менеджера подключён к этому API; см. [проверку RU/EN](../docs/DELIVERY_VERIFICATION.md). Существующий просмотр исходных данных сохранён. Никаких отправок поставщикам нет.
...
The backend implements source preparation, explained calculation, saved revisions, quantity overrides with reasons, explicit approval, and CSV export. The main manager interface is connected to this API; see [current verification](../docs/DELIVERY_VERIFICATION.md). The existing source browser is preserved; no supplier messages are sent.
...
The workflow suite checks migration upgrade, drift, downgrade, recreation, concurrency, restart persistence, immutable revisions, approval invalidation and export. The source suite checks version isolation, signed/missing/future movements, warehouse scope, source references and the composed FastAPI workflow. [Independent acceptance](../docs/BACKEND_ACCEPTANCE.md) records synthetic requirements and tolerances. [Parallel plan](../docs/PARALLEL_DELIVERY_PLAN.md) records task ownership and integration results.
