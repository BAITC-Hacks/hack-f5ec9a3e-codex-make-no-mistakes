...
class PlanningRequest(ContractModel):
    planning_date: date
    forecast_end: date | None = None  # Inclusive custom end; absent preserves lead + review policy.
    forecast_method: Literal["history_mean", "auto"] = "history_mean"
    lead_time_days: LeadDays | None = None
    ...
    def validate_rows_and_forecast_origin(self) -> Self:
        if self.forecast_end is not None:
            horizon = (self.forecast_end - self.planning_date).days + 1
            if not 1 <= horizon <= 730 or self.forecast_end == date.max:
                raise ValueError(
                    "Custom forecast period must contain 1 to 730 days and allow an exclusive end"
                )
