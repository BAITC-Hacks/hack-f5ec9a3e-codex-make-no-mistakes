...
def prepare_source_input(connection, metadata, request):
    ...
    for product_id in request.product_ids:
        ...
        history_start = request.history_start or request.planning_date - timedelta(days=56)
        history_end = request.history_end or request.planning_date - timedelta(days=1)
        ...
        if len(movement_books) > 1 or len(units) != 1:
            ...
            selected = [r for r in movements if
                        ...
                        < datetime.combine(history_end + timedelta(days=1), time.min)]
