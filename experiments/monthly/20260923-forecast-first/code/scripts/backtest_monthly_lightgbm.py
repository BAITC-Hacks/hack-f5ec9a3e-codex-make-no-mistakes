...
def main():
    ...
    code_files = [Path(__file__), ROOT / "experiments/requirements.txt",
                  *sorted((ROOT / "backend/src/replenishment/calculation").glob("*.py")),
                  ...
                  ROOT / "backend/tests/evaluation/cases.py", ROOT / "backend/tests/evaluation/contracts.py"]
    ...
    report = {
        ...
        "code_sha256": {
            path: hashlib.sha256(content).hexdigest() for path, content in code_snapshot.items()
        },
        "reproduction_command": "experiments/.venv/bin/python scripts/backtest_monthly_lightgbm.py --output <new-directory>",
    }
