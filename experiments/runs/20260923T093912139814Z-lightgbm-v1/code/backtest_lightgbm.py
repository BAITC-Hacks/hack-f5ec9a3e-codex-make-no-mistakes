...
def main():
            ...
            for horizon in (7, 28):
                ...
                for origin, offset in zip(origins, offsets):
                    ...
                    for model_id, objective in CANDIDATES.items():
                        tick = perf_counter()
                        dataset = lgb.Dataset(xtrain[mask], label=ytrain[mask], feature_name=names,
                                              categorical_feature=["sku_id"])
                        model = lgb.train({**PARAMS, **objective}, dataset, num_boost_round=ROUNDS)
                        forecasts[model_id] = np.maximum(model.predict(xp, num_threads=2), 0)
                        ...
                        if origin == origins[-1]:
                            importance.extend({"supplier": folder.name, "unit": unit, "horizon": horizon,
                                              ...
                                              for name, gain in zip(names, model.feature_importance("gain")))
    ...
    summary = {"protocol": "Track B v1; direct horizon totals; fixed two-objective grid",
