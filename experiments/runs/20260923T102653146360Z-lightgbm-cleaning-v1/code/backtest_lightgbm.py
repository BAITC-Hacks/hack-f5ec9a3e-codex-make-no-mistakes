...
def main():
    ...
    if args.ablation and args.cleaning:
        parser.error("Choose ablation or cleaning, not both")
    ...
    for folder in sorted((base.ROOT / "docs/data").iterdir()):
        source_rows = [] if args.cleaning else None
        ...
        if args.cleaning:
            ...
            for offset in all_offsets:
                cleaned[offset] = cleaner.at(base.START + timedelta(days=offset))[0]
        ...
        for unit in sorted({k[1] for k in keys}):
            ...
            for offset in all_offsets:
                ...
                if args.cleaning:
                    clean_features[offset] = features(cleaned[offset][indexes][eligible], eligible,
                                                      base.START + timedelta(days=offset))[0]
            for horizon in (7, 28):
                ...
                for origin, offset in zip(origins, offsets):
                    ...
                    for model_id, objective in candidates.items():
                        ...
                        model_train = xtrain_clean if model_id == "tweedie_clean" else xtrain
                        model_predict = clean_features[offset] if model_id == "tweedie_clean" else xp
    ...
    summary = {"protocol": ("Issue #3; paired Tweedie raw/clean features; raw labels" if args.cleaning else
